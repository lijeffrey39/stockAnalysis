...
def neuralnet():
        ...
        for date in dates:
            for feature in features[stock][date]:
                if feature not in featureList:
                    del newfeatures[stock][date][feature]
    ...
    for stock in stocks:
        for date in dates:
            # print(len(newfeatures[stock][date]))
            for feature in newfeatures[stock][date]:
                mean = result[stock][feature]["mean"]
                stdev = result[stock][feature]["stdev"]
                newfeatures[stock][date][feature] = (newfeatures[stock][date][feature] - mean)/stdev
    ...
    for stock in stocks:
        for date in dates:
            ...
            if temp[2] < 0:
                ...
                label = [1,0]
            trainingData.append((np.asarray(list(newfeatures[stock][date].values())),np.asarray(label)))
    ...
    for stock in stocks:
        for date in testingDates:
            ...
            if temp[2] < 0:
                ...
                label = [1,0]
            testingData.append((np.asarray(list(newfeatures[stock][date].values())),np.asarray(label)))
